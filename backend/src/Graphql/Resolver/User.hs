...
-------------------------------------------------------------------------------
userResolver :: GraphQL o => DB.User -> Value o Graphql.User
userResolver user =
...
-------------------------------------------------------------------------------
loginResolver :: GraphQL o => LoginArgs -> Value o Graphql.Session
loginResolver LoginArgs { email, password } = do
  res :: [DB.User] <- runSelect $ DB.findUserByEmail email
  case res of
    [user]
...
-------------------------------------------------------------------------------
registerResolver :: RegisterArgs -> Value MUTATION Graphql.Session
registerResolver RegisterArgs { email, password, name } = do
  ...
  case res of
    ...
    []    -> do
      ...
      runInsert $ DB.insertUser (email, ph, name)
      loginResolver LoginArgs { email, password }

-------------------------------------------------------------------------------
myUserInfoResolver :: Value QUERY Graphql.User
myUserInfoResolver = do
  myUserId <- requireAuthorized
  runSelectOne (DB.findUserByID myUserId) "Invalid user" >>= userResolver
...
changePasswordResolver ChangePasswordArgs { oldPassword, newPassword } = do
  myUserId            <- requireAuthorized
  userData :: DB.User <- runSelectOne (DB.findUserByID myUserId) "Invalid user"
  if validateHashedPassword (DB.userPasswordHash . record $ userData)
                            oldPassword
    then do
      ph <- liftIO $ hashPassword newPassword
      runUpdate $ DB.updateUserPassword myUserId ph
      return True
