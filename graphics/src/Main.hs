...
main :: IO ()
main = C.withSDL $
    C.withWindow "HexTech" (1000, 800) $ \w ->
...
-}
renderScaled :: (MonadIO m) => SDL.Renderer -> SDL.Surface -> SDL.Surface -> m ()
renderScaled r s t
...
makeGrid = map tile coords
    where
      ...
      tile (i, j) = makeHexagon
            ( round $ x + xOffset * j + (size * fromIntegral (i `rem` 2 :: Int)) - 2 * lineWidth
            , round $ y + yOffset * fromIntegral i - 2 * lineWidth
            ) $ round size
      coords = [(i, j) | i <- [0..10], j <- [0..10]]
...
draw r = do
  ...
  foldM (\_ hexagon -> (withColor Green  >> (drawLines r $ hexagon))) () $ makeGrid
...
drawLines :: (MonadIO m) => SDL.Renderer -> Vector (SDL.Point SDL.V2 CInt) ->  m ()
drawLines r points =
    SDL.drawLines r points
